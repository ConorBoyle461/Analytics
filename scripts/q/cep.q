...
parms:1#.q ;
parms:(.Q.def[`tpPort`action`log!("5000";"START";(getenv `LOGDIR),"processlogs/cep.log");.Q.opt .z.x]),.Q.opt[.z.x];
...
if[all parms[`action] like "START"; 
  ...
  upd:{[t;x] t upsert x}  ;                                                     /Initial definition of upd so tpLogs can be read in and brought back up to sync with tp
  .u.rep .({handle(`.u.sub;x;`)} each `$parms[`tables];handle(`.u.i);handle(`.u.L )) ;
  upd:{[t;x] .log.write "Update received for table ",string t; 
        t upsert x;                                /Redefining upd to aggregate publishing func after tp logs read in
        if[`trade = t;  
            agg_data:`time`sym`max_price`min_price`volume xcols 0!select time:"n"$.z.N, max_price:`float$max price, min_price:`float$min price, volume:`int$sum size by sym from trade;
      if[0<count agg_data;.log.write "Publishing aggregation data to TP"; handle(`.u.upd;`aggregation;agg_data)]]};
  ];
